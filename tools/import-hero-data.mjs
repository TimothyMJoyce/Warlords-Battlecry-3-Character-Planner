...
import { portableArchiveLabel, resolveHeroDataArchivePath } from "./wbc3-paths.mjs";

const heroDataPath = await resolveHeroDataArchivePath(process.argv[2]);
const outputPath = resolve(import.meta.dirname, "../src/data/importedHeroBuilds.js");
...
for (const heroFileName of index.heroFileNames) {
  ...
  const heroBytes = readResource(archive, resource);
  importedHeroBuilds.push(parseHero(heroBytes, resource.name));
}

const metadataForJs = JSON.stringify(
  {
    path: portableArchiveLabel(heroDataPath, "HeroData.xcr"),
    archive: portableArchiveLabel(heroDataPath, "HeroData.xcr"),
    resourceCount: resources.length,
    heroCount: importedHeroBuilds.length,
  },
  null,
  2,
);

const buildsForJs = JSON.stringify(importedHeroBuilds, null, 2);

await writeFile(
  outputPath,
  `export const importedHeroBuilds = ${buildsForJs};\n\nexport const importedHeroBuildMetadata = ${metadataForJs};\n`,
);

console.log(`Imported ${importedHeroBuilds.length} hero builds from ${heroDataPath}`);
